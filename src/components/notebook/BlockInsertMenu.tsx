  ...
  return (
    <div ref={menuRef} className="relative">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 rounded-lg border border-dashed border-white/30 px-3 py-2 text-sm text-white/70 transition-colors hover:border-white/50 hover:text-white"
      >
        <span aria-hidden>+</span>
        {label}
      </button>
      {open && (
        <div className="absolute left-0 top-full z-10 mt-1 min-w-[160px] rounded-lg border border-white/10 bg-neutral-800 py-1 shadow-xl">
